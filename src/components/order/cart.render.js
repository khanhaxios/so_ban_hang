    ...
    const CartItem = memo(({item}) => {
        return (
            <HStack
                ...
                alignItems="center"
            >
                <Text fontSize="md" flex={1} width={'40%'} numberOfLines={2}>
                    {item.name}
                </Text>
                <Text fontSize="md" width={'30%'} fontWeight={'bold'}>{formatCurrency(item.price)}</Text>
    ...
    return (
        <VStack width="35%" bg="white" p={4}>
            {cartModel.cartItems.length > 0 ? (
                <VStack space={3}>
                    <FlatList data={cartModel.cartItems} renderItem={({item}) => <CartItem item={item}/>}/>
                    <Divider my={2}/>
                    <HStack justifyContent="space-between">
                        <Text fontSize="md">
                            Tổng cộng {cartModel.cartItems.length} sản phẩm
                        </Text>
                        <Text fontSize="md" fontWeight={'bold'}>{formatCurrency(totalAmount)} đ</Text>
                    </HStack>
